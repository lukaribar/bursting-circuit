...
class GUI:
    ...
    def __init__(self, system, **kwargs):
        ...
        self.V_extended = np.arange(
            self.vmin - vrange / 2, self.vmax + vrange / 2, self.dv
        )
    ...
    def update_IV_curves(self):
        ...
        if self.plot_fixed_point and (self.vmin < self.v_rest < self.vmax):
            self.axs_iv[-1].plot(
                self.v_rest, self.I_ss_rest, "C2", marker=".", markersize=10
            )
    ...
    def add_iapp_slider(self, coords, val_min, val_max):
        slider = self.add_slider(
            "$I_{app}$", coords, val_min, val_max, self.i0, self.update_iapp
        )
        return slider
